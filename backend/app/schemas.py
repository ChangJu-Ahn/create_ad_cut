...
class GalleryThumbnail(BaseModel):
    """Lightweight generation projection used by the gallery card grid."""
...
class GallerySessionSummary(BaseModel):
    """Per-session card payload for the gallery list view.

    All fields tolerate missing legacy data and never raise; missing values
    surface as `None`/empty lists so the UI can render empty/placeholder states.
    """
    ...
    thumbnails: list[GalleryThumbnail] = Field(
        ...
        description="Up to 4 most recent generated images for the card preview grid.",
    )
...
class GalleryListOut(BaseModel):
    items: list[GallerySessionSummary]
